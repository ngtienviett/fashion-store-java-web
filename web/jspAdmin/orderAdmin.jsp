<%-- 
    Document   : orderAdmin
    Created on : Nov 10, 2020, 2:11:10 AM
    Author     : GearVn
--%>
    ...
    <body>
        ...
        <div class="container-fluid" style="padding: 2% 10%;text-align: center;vertical-align: central;text-justify: auto">
            ...
            <table border="1" cellspacing="10px" class="table">
                ...
                <c:forEach var="c" items="${orderAdmin}">
                <tr>
                    <td><a href="orderDetailAdmin?Id=${c.getId()}">${c.getId()}</a></td>
                    <td>${c.getUsername()}</td>
                    ...
                    <c:set var="x" value="${'Pending'}"/>
                    <c:if test="${(c.getStatus() eq x)}" >
                        <th>
                        <a href="ConfirmOrder?Id=${c.getId()}"><input type="submit" class="btn btn-info" value="Confirm"/></a> 
                        <a href="CancelOrder?Id=${c.getId()}"><input type="submit" class="btn btn-danger" value="Cancel"/></a>
                    </th>
                    </c:if>
